...
// MARK: - 生成页（模式自动判定：有参考音频=克隆，无=纯文本）
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ZStack {
                ...
                if app.speakerFile.isEmpty {
                    VStack(spacing: 8) {
                        Image(systemName: "person.wave.2").font(.title2).foregroundStyle(.tertiary)
                        Text("不提供 = 纯文本合成（默认音色）\n拖入或录一段人声 = 声音克隆（模仿该声音）\n（wav / mp3 / m4a / flac）")
                ...
                VStack(spacing: 8) {
                    HStack(spacing: 8) {
                        ...
                        Slider(value: positionBinding, in: 0...max(app.speakerDuration, 0.01))
                            .disabled(app.speakerDuration <= 0 || (app.playingFile != nil && app.playingFile != app.speakerFile))
                            .opacity(app.playingFile != nil && app.playingFile != app.speakerFile ? 0.4 : 1)
                        Text(String(format: "%.2fs", app.speakerDuration))
                            .font(.caption.monospacedDigit())
